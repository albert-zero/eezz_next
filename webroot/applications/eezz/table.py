...
from   typing      import List, Dict, NewType, Tuple, Any
...
class TTable(collections.UserList):
    ...
    def do_find(self, column_name: str, cell_value: Any) -> TTableRow | None:
        try:
            x_index = self.column_names.index(column_name)
        except KeyError:
            return None

        for x_row in self.data:
            if x_row.cells[x_index].value == cell_value:
                return x_row
        return None
